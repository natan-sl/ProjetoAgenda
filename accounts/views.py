...
def cadastro(request):
    ...
    if len(senha) < 8:
        messages.error(request, 'A senha precisa ter no minimo oito caracteres')
        return render(request, 'accounts/cadastro.html')
    if len(usuario) < 6:
        messages.error(request, 'O usuário precisa ter no minimo oito caracteres')
        return render(request, 'accounts/cadastro.html')
    if senha != senha2:
...
def dashboard(request):
    ...
    formulario = FormContato(request.POST, request.FILES)
